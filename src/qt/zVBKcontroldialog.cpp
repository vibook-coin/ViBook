...
#include "ui_zVBKcontroldialog.h"
...
std::list<std::string> ZVBKControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVBKControlDialog::listMints;

ZVBKControlDialog::ZVBKControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVBKControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVBKControlDialog::~ZVBKControlDialog()
{
    delete ui;
}

void ZVBKControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVBKControlDialog::updateList()
{
...
void ZVBKControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVBKControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVBK_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVBKControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVBKControlDialog::GetSelectedMints()
{
...
void ZVBKControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
