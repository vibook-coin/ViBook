...
public:
    ...
    void setupUi(QDialog *SendCoinsDialog)
    {
        ...
        labelCoinControlAfterFee->setText(QStringLiteral("0.00 VBK"));
        ...
        labelCoinControlChange->setText(QStringLiteral("0.00 VBK"));
        ...
        checkzVBK = new QCheckBox(SendCoinsDialog);
        checkzVBK->setObjectName(QStringLiteral("checkzVBK"));
        checkzVBK->setEnabled(false);

        horizontalLayout_2->addWidget(checkzVBK);
        ...
        labelBalance->setText(QStringLiteral("123.456 VBK"));
    ...
    void retranslateUi(QDialog *SendCoinsDialog)
    {
        ...
        labelBlockSize->setText(QApplication::translate("SendCoinsDialog", "0 VBK", 0));
...
#ifndef QT_NO_TOOLTIP
        radioCustomPerKilobyte->setToolTip(QApplication::translate("SendCoinsDialog", "If the custom fee is set to 1000 uVBKs and the transaction is only 250 bytes, then \"per kilobyte\" only pays 250 uVBKs in fee,<br />while \"at least\" pays 1000 uVBKs. For transactions bigger than a kilobyte both pay by kilobyte.", 0));
#endif // QT_NO_TOOLTIP
        radioCustomPerKilobyte->setText(QApplication::translate("SendCoinsDialog", "per kilobyte", 0));
#ifndef QT_NO_TOOLTIP
        radioCustomAtLeast->setToolTip(QApplication::translate("SendCoinsDialog", "If the custom fee is set to 1000 uVBKs and the transaction is only 250 bytes, then \"per kilobyte\" only pays 250 uVBKs in fee,<br />while \"total at least\" pays 1000 uVBKs. For transactions bigger than a kilobyte both pay by kilobyte.", 0));
#endif // QT_NO_TOOLTIP
        radioCustomAtLeast->setText(QApplication::translate("SendCoinsDialog", "total at least", 0));
#ifndef QT_NO_TOOLTIP
        checkBoxMinimumFee->setToolTip(QApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks.<br />But be aware that this can end up in a never confirming transaction once there is more demand for VBK transactions than the network can process.", 0));
#endif // QT_NO_TOOLTIP
        checkBoxMinimumFee->setText(QString());
#ifndef QT_NO_TOOLTIP
        labelMinFeeWarning->setToolTip(QApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks.<br />But be aware that this can end up in a never confirming transaction once there is more demand for VBK transactions than the network can process.", 0));
#endif // QT_NO_TOOLTIP
        ...
        checkzVBK->setText(QApplication::translate("SendCoinsDialog", "Anonymized VBK", 0));
