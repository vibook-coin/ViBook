                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zVBK ought to be enough for anybody." - Bill Gates, 2017
    ui->zVBKpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVBKSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzVBK->setEnabled(false);
        ui->pushButtonMintzVBK->setToolTip(tr("zVBK is currently disabled due to maintenance."));

        ui->pushButtonSpendzVBK->setEnabled(false);
        ui->pushButtonSpendzVBK->setToolTip(tr("zVBK is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVBKpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzVBK_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVBK is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVBK...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVBK in ") +
...
void PrivacyDialog::on_pushButtonSpendzVBK_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVBK is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zVBK
        sendzVBK();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVBK
    sendzVBK();
}

void PrivacyDialog::on_pushButtonZVBKControl_clicked()
{
    ZVBKControlDialog* zVBKControl = new ZVBKControlDialog(this);
    zVBKControl->setModel(walletModel);
    zVBKControl->exec();
}

void PrivacyDialog::setZVBKControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVBKSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVBK()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid VBK Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVBKpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVBK is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VBK </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVBK</b>";
    ...
    // use mints from zVBK selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVBKControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVBKControlDialog::GetSelectedMints();
    }
    ...
    // Clear zVBK selector in case it was used
    ZVBKControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVBK Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " VBK, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVBK Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVBK </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVBK "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVBK "));
    ui->labelzVBKAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzVBKSyncStatus->setVisible(fShow);
}
